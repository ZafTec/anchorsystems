...
-- This schema adds support for conversation tracking and token usage monitoring

-- Conversations table to track chat sessions
CREATE TABLE IF NOT EXISTS conversations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id VARCHAR(255), -- Optional: For anonymous user tracking
    user_id VARCHAR(255), -- Optional: For authenticated users in the future
    title TEXT, -- Optional: First message or user-defined title
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    metadata JSONB DEFAULT '{}'::jsonb -- Store additional context like user agent, IP, etc.
);
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    metadata JSONB DEFAULT '{}'::jsonb -- Store additional info like model used, etc.
);
...
CREATE TABLE IF NOT EXISTS token_usage (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    conversation_id UUID NOT NULL REFERENCES conversations(id) ON DELETE CASCADE,
    message_id UUID REFERENCES messages(id) ON DELETE CASCADE, -- Link to specific message if applicable
    model VARCHAR(100) NOT NULL, -- e.g., 'gemini-2.5-flash-lite'
    prompt_tokens INTEGER NOT NULL DEFAULT 0,
    completion_tokens INTEGER NOT NULL DEFAULT 0,
    total_tokens INTEGER NOT NULL DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    metadata JSONB DEFAULT '{}'::jsonb -- Store additional metrics
);
